    ...
    def move(self, start, end):
        ...
        for i in range(12):
            ...
            if self.get_bit(self.pos[i], start) == 1:
                ...
                if i == 0 or i ==1: # for en passent and promotion
                    if abs(start-end)==16: #for en passent
                        self.pos[15] = 0
                        self.pos[15] = self.set_bit(self.pos[15],int((start+end)/2)) # enables en passent
                        self.pos[15] = self.set_bit(self.pos[15],end)
                    elif 1<<end & self.pos[15] != 0: #checks if p
                    ...
                    if 1<<end & 18374686479671623935 != 0: #for promotion
                        self.pos[i+8] = self.set_bit(self.pos[i+8],end) 
                        self.pos[i] = self.remove_bit(self.pos[i],end)
                        self.update_bitboards()
